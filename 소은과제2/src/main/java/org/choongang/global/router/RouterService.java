...
public class RouterService {
    ...
    private final HandlerMappingImpl handlerMapping; // 컨트롤러와 메서드 찾는것
    private final HandlerAdapterImpl handlerAdapter; // 실행시켜주는 것
    private final StaticResourceMappingImpl staticResourceMapping; // 없으면 정적 경로를 찾으러 감
    private final ExceptionHandlerService exceptionHandlerService; // 찾아도 없으면 에러페이지 실행
    ...
    public void route(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
        ...
        try {
            ...
            req.setAttribute("message", e.getMessage()); // 속성으로 조회
            req.setAttribute("exception", e); // 속성으로 조회
            if (e instanceof ServletException || e instanceof IOException) { // 기본적인 예외가 발생했을때는 500에러페이지 출력
            ...
            // 예외 페이지 처리
            if (data != null) { // 우리가 정의한 예외는 예외에 따라 페이지를 정의해서 출력
